...
app.get('/', (req, res) => {
    ...
    fs.readdir(SERVER_FOLDER, (err, files) => {
        ...
        let textMtime = 0;
        try { if (fs.existsSync(TEXT_DB_FILE)) textMtime = fs.statSync(TEXT_DB_FILE).mtimeMs; } catch {}
        const initialState = { fileCount: fileData.length, maxMtime, settings, totalSizeBytes, textMtime };
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-container { display: flex; gap: 12px; margin-top: 12px; width: 100%; }
...
<div class="app-container">
    ...
    <div class="text-layout" style="display: none; width: 100%; gap: 20px;">
        <div class="box" style="width: 100%; min-height: 70vh; display: flex; flex-direction: column;">
            <h2 style="margin-bottom: 20px;">
                <span><span style="color: #ccff00;">//</span> TEXT_DATABASE</span>
                <span style="font-weight:600;color:#666;font-size:0.75rem;font-family:monospace;border:1px solid #444;padding:2px 6px;letter-spacing:0;">database.txt</span>
            </h2>
            <textarea id="textData" style="flex: 1; width: 100%; background: #0a0a0a; color: #ccff00; border: 1px solid #333; padding: 16px; font-family: monospace; font-size: 1rem; resize: none; outline: none; transition: border 0.3s; margin-bottom: 16px;" placeholder="Write or paste your long text here..."></textarea>
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <div id="text-notification" style="display:none; padding: 8px 12px; font-size: 0.85rem; font-family: monospace; border-left: 3px solid;"></div>
                <button id="saveTextBtn" class="upload-btn" onclick="saveTextData()" style="width: 200px; margin-top: 0;">SAVE TEXT</button>
            </div>
        </div>
    </div>
</div>
    ...
    <script>
        ...
        let currentTextMtime = initialState.textMtime || 0;
        ...
        setInterval(async () => {
            ...
            try {
                ...
                if (state.textMtime !== currentTextMtime) {
                    if (document.activeElement !== document.getElementById('textData')) {
                        currentTextMtime = state.textMtime;
                        loadTextData();
                    }
                }
        ...
        function switchTab(tab) {
            if (tab === 'file') {
                document.getElementById('nav-file').classList.add('active-nav');
                document.getElementById('nav-text').classList.remove('active-nav');
                document.querySelector('.main-layout').style.display = 'grid';
                document.querySelector('.text-layout').style.display = 'none';
            } else {
                document.getElementById('nav-text').classList.add('active-nav');
                document.getElementById('nav-file').classList.remove('active-nav');
                document.querySelector('.main-layout').style.display = 'none';
                document.querySelector('.text-layout').style.display = 'flex';
                loadTextData();
            }
        }
        ...
        async function loadTextData() {
            try {
                const res = await fetch('/api/text?t=' + Date.now());
        ...
        async function saveTextData() {
            ...
            const notif = document.getElementById('text-notification');
            btn.disabled = true;
            ...
            try {
                ...
                if (data.success) {
                    ...
                    notif.textContent = '[OK] TEXT SAVED TO DATABASE.TXT';
                    notif.style.display = 'block';
                    setTimeout(() => notif.style.display = 'none', 3000);
                } else {
                    notif.className = 'notif-error';
                    notif.textContent = '[ERROR] FAILED TO SAVE TEXT';
                    notif.style.display = 'block';
                }
            } catch (e) {
                notif.className = 'notif-error';
                notif.textContent = '[ERROR] NETWORK ERROR';
                notif.style.display = 'block';
            }
            btn.disabled = false;
...
app.get('/api/state', (req, res) => {
    try {
        ...
        let textMtime = 0;
        try { if (fs.existsSync(TEXT_DB_FILE)) textMtime = fs.statSync(TEXT_DB_FILE).mtimeMs; } catch {}
        res.json({ fileCount: files.length, maxMtime, textMtime });
    } catch {
        res.json({ fileCount: 0, maxMtime: 0, textMtime: 0 });
    }
});
...
app.get('/api/text', (req, res) => {
    ...
    if (fs.existsSync(TEXT_DB_FILE)) {
        text = fs.readFileSync(TEXT_DB_FILE, 'utf8');
    }
    res.json({ text });
});
...
app.post('/api/text', (req, res) => {
    ...
    const textStr = text || '';
    const settings = loadSettings();
    const maxBytes = (settings.maxTextSizeMB || 1) * 1024 * 1024;
    if (Buffer.byteLength(textStr, 'utf8') > maxBytes) {
        return res.status(400).json({ success: false, error: `Ukuran teks melebihi batas maksimal ${settings.maxTextSizeMB || 1} MB` });
    }
    fs.writeFileSync(TEXT_DB_FILE, textStr);
    res.json({ success: true });
});
